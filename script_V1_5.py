...
def main():
    ...
    x_train, x_test = split_data(images)

    # Optimize using Optuna
    study = optuna.create_study(direction="minimize")
    study.optimize(lambda trial: objective(trial, x_train, x_test, epochs), n_trials=50)
    ...
    history = train_model(best_model, x_train, x_test, epochs, best_batch_size)
...
# Encoder and decoder functions now accept n_filters as an argument
def create_encoder(n_filters):
    encoder = Sequential()
    encoder.add(Conv2D(n_filters[0], (3, 3), activation="elu", padding="same", input_shape=(64, 64, 3)))
    encoder.add(MaxPooling2D((2, 2), padding="same"))
    encoder.add(Conv2D(n_filters[1], (3, 3), activation="elu", padding="same"))
    encoder.add(MaxPooling2D((2, 2), padding="same"))
    encoder.add(Conv2D(n_filters[2], (3, 3), activation="elu", padding="same"))
    encoder.add(MaxPooling2D((2, 2), padding="same"))
    return encoder

def create_decoder(n_filters):
    decoder = Sequential()
    decoder.add(Conv2D(n_filters[2], (3, 3), activation="elu", padding="same", input_shape=(8, 8, n_filters[2])))
    decoder.add(UpSampling2D((2, 2)))
    decoder.add(Conv2D(n_filters[1], (3, 3), activation="elu", padding="same"))
    decoder.add(UpSampling2D((2, 2)))
    decoder.add(Conv2D(n_filters[0], (3, 3), activation="elu", padding="same"))
    decoder.add(UpSampling2D((2, 2)))
...
def train_model(model, x_train, x_test, epochs, batch_size):
    ...
    lr_scheduler = ReduceLROnPlateau(
        monitor='val_loss', factor=0.1, patience=5, verbose=2, mode='min',
            min_delta=0.0001, cooldown=0, min_lr=0.0000000001, max_lr=0.02
    )
    early_stopping = EarlyStopping(monitor="loss", patience=6)
    history = model.fit(x_train, x_train, epochs=epochs, batch_size=batch_size, validation_data=(x_test, x_test), callbacks=[lr_scheduler, early_stopping])
    return history
