...
from tensorflow.keras.layers import Conv2D, Dropout, AveragePooling2D, MaxPooling2D, UpSampling2D, Input, Conv2DTranspose, Dense
...
def visualize_and_evaluate_results(model, x_test, history):
    ...
    for i in tqdm(range(n), desc="Visualising and evaluating results", unit="images"):
        ...
        plt.title(f'Reconstructed\nAvg PSNR: {np.mean(psnrs):.4f}\nAvg SSIM: {np.mean(ssims):.4f}', fontsize=10)
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
